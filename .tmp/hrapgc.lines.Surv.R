lines.Surv <-
structure(function (x, type = "s", mark = 3, col = 1, lty = 1, lwd = 1, 
            mark.time = TRUE, xscale = 1, firstx = 0, firsty = 1, xmax, 
            fun, conf.int = FALSE, ...) 
{
  ## Purpose: Changed survival into mortality (use with plot.Surv)
  ## ----------------------------------------------------------------------
  ## Modified from: survival:::lines.survfit
  ## ----------------------------------------------------------------------
  ## Arguments: 
  ## ----------------------------------------------------------------------
  ...
  if (is.numeric(mark.time)) 
    mark.time <- sort(unique(mark.time[mark.time > 0]))
  if (is.matrix(x$surv)) {
    ncol.per.strat <- ncol(x$surv)
    ncurve <- ncol(x$surv)
    coffset <- nrow(x$surv) * (1:ncurve - 1)
  }
  else {
    ncol.per.strat <- 1
    ncurve <- 1
  ...
  for (i in 1:nstrat) {
    for (j in 1:ncol.per.strat) {
      ...
      yend <- c(yend, max(yy))
      if (plot.surv) {
        lines(dostep(xx, 1- yy), type = type, col = col[k], 
              lty = lty[k], lwd = lwd[k], ...)
        if (is.numeric(mark.time)) {
          indx <- mark.time
