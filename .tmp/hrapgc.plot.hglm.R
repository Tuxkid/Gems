plot.hglm <-
structure(function(x, pch = "+", col.theme = "colorful", output = "screen",
           filename = "HGLMplot", ...) {
    residuals <- x$resid
    fitted.values <- x$fv
...
##cook.dis <- deviances/(p*sum(deviances))*hatvalues/(1 - hatvalues)**2
    if (col.theme == "colorful") {
      ...
      if (col.theme == "blackwhite") {
        ...
        stop("Specify 'blackwhite' or 'colorful' for col.theme")
      }
    }
...
##        }
      ...
      if (i == 1) {
        ...
        plot(fitted.values, residuals, xlab = "Fitted Values", 
             ylab = "Studentized Residuals", pch = pch, col = pcol,
             bty = "n", main = "Mean Model (a)")
 ...
 ##         par(mfrow = c(2,2), pty = "s", ...)
          ...
          plot(disp.fitted.values, disp.residuals, xlab = "Fitted Values", 
               ylab = "Standardized Deviance Residuals", pch = pch,
               col = pcol, bty = "n", main = "Dispersion Model (a)")
          lines(loess.fit$x, loess.fit$y, col = lcol)
          ...
          plot(disp.fitted.values, abs(disp.residuals), xlab = "Fitted Values", 
               ylab = "|Standardized Deviance Residuals|", pch = pch,
               col = pcol, bty = "n", main = "Dispersion Model (b)")
          lines(loess.fit$x, loess.fit$y, col = lcol)
          qqnorm(disp.residuals, col = pcol, pch = pch, bty = "n", 
                 xlab = "Normal Quantiles", ylab = "Residual Quantiles",
                 main = "Dispersion Model (c)")
          qqline(disp.residuals, col = lcol)
          hist(disp.residuals, density = 15,
               xlab = "Standardized Deviance Residuals",
               main = "Dispersion Model (d)", col = pcol)
        }
        else {
          if (i == 3) {
##            par(mfrow = c(2,2), pty = "s", ...)
            ...
            sy <- sort(deviances)
            plot(x.alt, sy, main="", col = pcol, pch = pch, bty = "n",
                 ylab = "Deviance Quantiles", xlab="Gamma Quantiles")
##qqplot(rgamma(9999, alpha, 1/beta), deviances, col = pcol, pch = pch, xlab = "Gamma Quantiles", ylab = "Deviance Quantiles", bty = "n")
            abline(0, 1, col = lcol)
            hist(deviances, density = 15, xlab = "Deviances", main = "",
                 col = pcol)
          }
        }
      }
