simplot <- function(x, resp, tot, fun, cm.strategy = "adjust.later", cm = 0, cm.code = 
           NULL, cm.allcodes = NULL, xtit = "Time in Min", main = "", mkh = 0.025, 
           xfun = function(x) x, takelog = F, line = F, ab = c(NA, NA),
           clip = NULL, xlimits = c(0, 0), idTotal = NULL,
...
## browser()
  if(!is.null(idTotal)) # added 28/11/14: show total no of individuals
     add.text(.95, .1, tex = c(idTotal), col = "thistle", cex = 2, adj = 1)
