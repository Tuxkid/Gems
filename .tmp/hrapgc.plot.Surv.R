plot.Surv <-
structure(function (x, conf.int, mark.time = TRUE, mark = 3, col = 1, lty = 1, 
            lwd = 1, cex = 1, log = FALSE, xscale = 1, yscale = 1, firstx = 0, 
            firsty = 1, xmax, ymin = 0, fun, xlab = "", ylab = "", xaxs = "S", 
            bty = NULL, legend.text = NULL, legend.pos = 0, legend.bty = "n", 
            main = NULL, ...) 
{
  ## Purpose: Changed survival into mortality (use with lines.Surv)
  ## ----------------------------------------------------------------------
  ## Modified from: survival:::plot.survfit
  ## ----------------------------------------------------------------------
  ## Arguments: 
  ## ----------------------------------------------------------------------
  ## Author: Patrick Connolly, Creation date:  2 Apr 2007, 11:20
  mintime <- min(x$time)
  ...
  plot(tempx, tempy * yscale, type = "n", log = logax, xlab = xlab, 
       ylab = ylab, xaxs = xaxs, main = main, bty = bty, ...)
  if (yscale != 1) {
    if (logy) 
  ...
  for (j in unique(stemp)) {
    ...
    if (is.matrix(ssurv)) {
      ...
      if (conf.int && !is.null(supper)) {
        if (ncurve == 1) 
          lty[i] <- lty[i] + 1
        yy <- 1- c(firsty, supper[who])
        lines(dostep(xx, yy), lty = lty[i], col = col[i], 
              lwd = lwd[i])
        yy <- 1- c(firsty, slower[who])
        lines(dostep(xx, yy), lty = lty[i], col = col[i], 
              lwd = lwd[i])
      }
    }
  ...
  if (!is.null(legend.text)) {
    ...
    if (length(legend.pos) == 2) {
      ...
      lly <- yscale * 0.95
      llx <- max(tempx) - max(strwidth(legend.text)) - 
        6 * xc
    }
